...
export default function SuccessModal({ kpis, onClose, jobId }) {
  const navigate = useNavigate();
  const handleViewResults = () => {
    navigate(`/results?id=${jobId}`);
  };
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50">
      <div className="bg-green-100 p-6 rounded-lg text-center">
        ...
        <button className="mt-4 px-4 py-2 bg-green-500 text-white" onClick={handleViewResults}>Ver resultados</button>
      </div>
    </div>
  );
}
